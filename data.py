...
class ReconDataset(Dataset):
    def __init__(self, dims, src="imagenet"):
        ...
        match src:
            case "imagenet-r":
                self.ds = (
                    datasets.load_dataset('axiong/imagenet-r', split='test')
                    .cast_column("image", datasets.Image("RGB"))
                    .select_columns(["image"])
                )
            case "imagenet":
                self.ds = (
                    datasets.load_dataset('ILSVRC/imagenet-1k', split='test',
            ...
            case "imagenet":
                self.ds = (
                    datasets.load_dataset('PhilSad/celeba-hq-15k', split='test')
                    .cast_column("image", datasets.Image("RGB"))
                    .select_columns(["image"])
                )
            case _:
